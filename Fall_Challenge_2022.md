...
after that this pseudo code do all the work :
...
In this function, the game simulation is done for the two chromosomes of the two populations. Then, the "getScore()" function is called to evaluate the position of the player. The getScore() function will be used to evaluate the player's position (more information in the next section).
...
### getBestMoves() :
The "getBestMoves()" function is used to get the best moves from the best chromosome of the population. It works by iterating over the best chromosome and print the moves to standard output.
...
## Space for improvement for the next contests :
- Search extensively before starting to code. This would allow for a better understanding of the problem and the available solutions, leading to a more efficient and effective implementation.
